...
class RaceBarChart extends Component {
  ...
  createBarChart = async () => {
    ...
    const nameframes = groups(
      keyframes.map(([, data]) => data).flat(),
      (d) => d.name
    );
    const prev = new Map(
      nameframes.map(([, data]) => pairs(data, (a, b) => [b, a])).flat()
    );
    const next = new Map(nameframes.map(([, data]) => pairs(data)).flat());

    // Add margin to the whole chart
    const chart = select(node).attr("viewBox", [0, 0, width, height]);
    ...
    for (const keyframe of keyframes) {
      const transition = chart.transition().duration(duration).ease(easeLinear);
    ...
    function color(d) {
      ...
      const scale = scaleOrdinal(COLORS.palette);
