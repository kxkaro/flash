...
const HorizontalBarChart = ({ data, size, resize, color }: BarChart) => {
  ...
  // Vertical axis
  const yScale = scaleBand().range([0, chartHeight]).domain(categories);

  const xRect = (d: number, i: number) => offset.left + (xScale(0) || 0);
  const yRect = (d: number, i: number) =>
    0.05 * yScale.bandwidth() + (yScale(categories[i]) || 0);
